...
class AppContextDelegateMock: NSObject, AppContextDelegate {
    ...
    @objc func context(_ context: AppContext, didResolveMultiConnectWithSyncValue value: NSString,
    ...
    @objc func context(_ context: AppContext, didChangePeerAvailability peers: NSString) {
        peerAvailabilityChangedHandler(peers as String)
    }
    @objc func context(_ context: AppContext, didChangeNetworkStatus status: NSString) {
        networkStatusUpdated = true
        networkStatus = status as String
    ...
    @objc func context(_ context: AppContext,
                       didUpdateDiscoveryAdvertisingState discoveryAdvertisingState: NSString) {
        advertisingListeningState = discoveryAdvertisingState as String
    }
...
class AppContextTests: XCTestCase {
    ...
    func testJsonValue() {
        ...
        XCTAssertEqual(jsonValue(jsonDict as AnyObject) as String, jsonString)
    ...
    func testConnectReturnValueCorrect() {
        ...
        let json = jsonDictionaryFrom(result as String)
    ...
    @objc fileprivate func centralBluetoothManagerStateChanged(_ notification: Notification) {
        if notification.name.rawValue == Constants.NSNotificationName.centralBluetoothManagerDidChangeState {
    ...
    fileprivate func changeBluetoothState(to state: BluetoothHardwareState,
        ...
        bluetoothChangingStateGroup!.notify(queue:
            DispatchQueue.global(qos: .background), execute: {
...
extension AppContextTests : BluetoothHardwareControlObserverProtocol {
    ...
    func receivedBluetoothManagerNotification(withName bluetoothNotificationName: String) {
        if bluetoothNotificationName == Notification.Name.PowerChanged.rawValue {
