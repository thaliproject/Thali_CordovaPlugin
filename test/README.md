...
To test how the Thali Cordova Plug-in manages these modes on Android, follow the steps below:
...
1. Get [the modified Thali Native Test application](https://github.com/thaliproject/Thali_CordovaPlugin_BtLibrary/releases/tag/v0.2.8) for **device A**.
   ...
   sufficient as long as the device support acting as a BLE peripheral (BLE multiple advertisment
   support).
...
3. Build the Thali Test app (in this project). See the instructions above.
...
5. Apply the Doze or Standby test commands for **device B** as explained [here](http://developer.android.com/training/monitoring-device-state/doze-standby.html#testing_doze_and_app_standby).
