...
var test = tape({
  ...
  teardown: function (t) {
    ...
    Mobile('stopListeningForAdvertisements').callNative(function (err) {
      t.notOk(err, 'Should be able to call stopListeningForAdvertisments in ' +
        'teardown');
...
test('Can shift large amounts of data', function (t) {
  ...
  function shiftData(sock, reverseConnection) {
    ...
    if (reverseConnection) {
      ...
      // end already sending data. Without multiplex support we can't both talk
      // at the same time so wait for the other side to finish before sending
      // our data.
      ...
      // This one's more straightforward.. we're going to send first, read back
      // our echo and then echo out any extra data
