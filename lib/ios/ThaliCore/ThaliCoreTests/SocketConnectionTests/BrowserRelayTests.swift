...
class BrowserRelayTests: XCTestCase {
    ...
    func testIncomingTCPConnectionIncrementsVirtualSocketsAmount() {
        // Expectations
        var MPCFBrowserFoundAdvertiser: XCTestExpectation?
        var browserManagerConnected: XCTestExpectation?

        // Given
        // Start listening on fake node server (Browser side)
        ...
        let a = expectationWithDescription("test")
        waitForExpectationsWithTimeout(10, handler: nil)

        // Then
        XCTAssertEqual(browserRelayInfo.relay.virtualSocketsAmount,
                       1,
                       "BrowserRelay must have active virtual socket")
    }
