...
http://www.goland.org/thaliidentityexchangeprotocol/. This implementation uses
a state machine defined below based on (but not identical to) the smaller hash
state machine defined in the second link. The machines has only three user
exposed APIs, the constructor for a new instance, start and stop. Once the
machine is stopped it is junk and can't do any further work. A new machine will
have to be created.
 */
...
var logger = require('../utils/thaliLogger')('smallerHash');
...
SmallerHashStateMachine.Events = {
  ...
  // The test values are just used for internal testing, don't use them!
  WrongPeer: 'testWrongPeer',
...
SmallerHashStateMachine.ExitBecauseNotNeededError =
  new Error('Exited because not needed');
SmallerHashStateMachine.ExitBecauseGotValidationCode =
  new Error('Exited because we got a validation code!');
...
function getPeerIdPort(self, portRetrievalTime) {
  // Note: This method assumes that whomever is calling it has already checked
  // to see if we are not in Exit.
  var tableEntry = self.connectionTable.lookUpPeerId(
    self.peerIdentifier,
    portRetrievalTime
  );
  ...
  if (!tableEntry) {
    self.portListener = function(tableEntry) {
      self.smallHashStateMachine.foundPeerPort(
        self,
        tableEntry.muxPort,
        tableEntry.time
      );
...
function makeRequestParseResponse(
  self,
  port,
  portRetrievalTime,
  urlPath,
  ...
  logger.info('Making ' + urlPath + ' request to pkOther value ' +
    urlSafeBase64.encode(self.otherPkHashBuffer));
  ...
  self.currentHttpRequest = request.post({
    ...
    if (self.smallHashStateMachine.current !== state) {
      logger.error('Oops, we aren\'t in ' + state +
          ' anymore, we should have been aborted, ' +
    ...
    if (error) {
      logger.info('Got error creating request to ' + urlPath + ' with port ' +
        port + ' : ' + JSON.stringify(error));
    ...
    if (response.statusCode === 200 || response.statusCode === 400) {
      if (!body) {
        logger.info('We didn\'t get a response body from a ' + urlPath +
      ...
      var pkOtherBuffer =
        identityExchangeUtils.validatePkAndGetBase64Object(body.pkOther);
      if (!pkOtherBuffer) {
        logger.info('We didn\'t get a pkOther value from a ' + urlPath +
          ' response with status ' + response.statusCode);
      ...
      if (identityExchangeUtils.compareEqualSizeBuffers(
        self.otherPkHashBuffer, pkOtherBuffer) !== 0) {

        logger.info('Got the wrong pkOther value from a ' + urlPath +
          ' response with status ' + response.statusCode);
      ...
      switch (body.errorCode) {
        ...
        case identityExchangeUtils.fourHundredErrorCodes.notDoingIdentityExchange:
          ...
          self.smallHashStateMachine.identityExchangeNotStarted(self);
          self.emit(
            SmallerHashStateMachine.Events.GotNotDoingIdentityExchange,
            urlPath
          );
...
function onFoundPeerPort(event, from, to, self, port, portRetrievalTime) {
  ...
  var cbValueBase64 = urlSafeBase64.encode(identityExchangeUtils.generateCb(
    rnMineBuffer,
    self.myPkHashBuffer,
  ...
  var requestBody =  {
    ...
    pkMine: urlSafeBase64.encode(self.myPkHashBuffer)
  };

  var validate200 = function(body) {
    var rnOtherBuffer = identityExchangeUtils.validateRnAndGetBase64Object(
      body.rnOther
    );
    ...
    if (!rnOtherBuffer) {
      ...
      self.smallHashStateMachine.channelBindingError(self, portRetrievalTime);
      self.emit(
        SmallerHashStateMachine.Events.BadRequestBody,
        identityExchangeUtils.cbPath
      );
      return;
    }
    ...
    self.smallHashStateMachine.cbRequestSucceeded(
      self,
      port,
      portRetrievalTime,
      rnMineBuffer,
  ...
  return makeRequestParseResponse(
    self,
    port,
    portRetrievalTime,
    identityExchangeUtils.cbPath,
...
function onCbRequestSucceeded(
  ...
  var requestBody = {
    rnMine: urlSafeBase64.encode(rnMineBuffer),
    pkMine: urlSafeBase64.encode(self.myPkHashBuffer)
  };
...
function onChannelBindingError(event, from, to, self, portRetrievalTime) {
  // We stop and start the replication manager to kill all connections since
  // we have a channel being error. Obviously this is thermo nuclear level
  // overkill since we just want to kill a single connection.
  identityExchangeUtils.stopThaliReplicationManager(
      self.thaliReplicationManager
    )
    .thenIfNotInExit(self, function() {
      return identityExchangeUtils
        .startThaliReplicationManager(
          self.thaliReplicationManager,
          self.port,
          self.dbName,
          self.deviceName
      ...
      }).catchIfNotInExit(self, function(err) {
        self.smallHashStateMachine.exitCalled(
          self,
          new Error('Could either not start or stop thali replication manager - ' + JSON.stringfy(err))
        );
      });
}
...
SmallerHashStateMachine.prototype.start = function() {
  if (identityExchangeUtils.compareEqualSizeBuffers(
    ...
    this.smallHashStateMachine.exitCalled(
      this,
      SmallerHashStateMachine.ExitBecauseNotNeededError
    );
  } else {
...
function SmallerHashStateMachine(
  thaliReplicationManager,
  connectionTable,
  peerIdentifier, otherPkHashBuffer,
  myPkHashBuffer,
  port,
  dbName,
  deviceName) {
  ...
  this.smallHashStateMachine = StateMachine.create({
    ...
    events: [{
      ...
      name: 'exitCalled',
      from: ['none', 'GetPeerIdPort', 'MakeCbRequest', 'WaitForIdentityExchangeToStart', 'MakeRnMineRequest', 'Exit'],
      to: 'Exit'
    }, {
      name: 'foundPeerPort',
      from: ['GetPeerIdPort', 'WaitForIdentityExchangeToStart'],
      to: 'MakeCbRequest'
    }, {
      name: 'identityExchangeNotStarted',
      from: ['MakeCbRequest', 'MakeRnMineRequest'],
      to: 'WaitForIdentityExchangeToStart'
    }, {
      name: 'cbRequestSucceeded',
      from: 'MakeCbRequest',
      to: 'MakeRnMineRequest'
    }, {
