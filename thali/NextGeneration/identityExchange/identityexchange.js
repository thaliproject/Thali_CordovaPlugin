...
function onStartIdentityExchangeCalled(
  ...
  if (!myFriendlyName || typeof myFriendlyName !== 'string' ||
    ...
    cb(new Error('myFriendlyName MUST be a string that is between 1 and 20 ' +
  ...
  /*
   It isn't legal to call executeIdentityExchange until after the cb has
   returned from onStartIdentityExchange. But this creates a race condition
   where we could emit a PeerIdentityExchange event causing someone to want to
   call executeIdentityExchange before we have had a chance to return from the
   ...
   peers we find while we wait for the replication manager to get going and then
   return them once everything is set up.
   */
  var storedPeers = [];
  ...
  return identityExchangeUtils.getDeviceIdentityFromThaliReplicationManager(
    ...
    .then(function (deviceName) {
      ...
      // Technically we should emit the events after we call the user's callback
      // so that the user always knows that their start method will get called
      // back before they can receive any PEER_AVAILABILITY_CHANGED events. But
      // it gives me the hives to let random user code run (via the callback) in
      // the middle of our code. So I'm intentionally allowing the race
      // condition where as soon as the start method is called, but before the
      // callback is called, PEER_AVAILABILITY_CHANGED events can occur.
      if (cb) {
        cb(null);
      }
    }).catch(function (err) {
...
function IdentityExchange(
  ...
  this.identityExchangeStateMachine = StateMachine.create({
    ...
    callbacks: {
      ...
      onstopExecutingIdentityExchangeCalled: 
                                           onStopExecutingIdentityExchangeCalled
    }
  });
}
